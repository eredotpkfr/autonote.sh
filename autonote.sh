...
function delete_files(){
    ...
    if [ ! -z "$deleted_files" ]; then
        zenity --question --width 333 --text="Are you sure ? Do you want to \
		delete this/these files ?\n $(echo -n $deleted_files | tr ' ' '\t')"
        if [ $? -eq 0 ]; then
...
function push_files(){
    ...
    if [[ "`git status`" == *"nothing to commit"* ]]; then
        if [[ "`git status`" == *"nothing to commit"* && "`git status`" == \
		*"(use \"git push\" to publish your local commits)"* ]]; then
